...
1. *MSH2* - **mavedb 00000050-a-1** (activity as readout)
...
g_pten_stability <- plotDMS(pten_dms_stability, clinvar_var = pten_clinvar, tcga_var = tcga,
        gene = "PTEN", title = "PTEN abudance", score_col = "score_stability", 
        x_limits = c(-0.2, 1.4))
...
g_msh2 <- plotDMS(msh2_dms, clinvar_var = msh2_clinvar, tcga_var = tcga,
        gene = "MSH2", title = "MSH2 activity", score_col = "score", 
        x_limits = c(-6, 8))
...
dms_sig_regression <- list(
  ...
  plotGLMCoef(glm(data = unique(pten_dms_stability[which(pten_dms_stability$variable == "any"),]), formula = score_scaled ~ 0 + MutSig)) +
    ggtitle("PTEN abundance") + ylim(-1.7, 1.7)
)

dms_sig_regression <- lapply(dms_sig_regression, function(x) x$data)
dms_sig_regression[[1]]$assay <- "MSH2 activity"
dms_sig_regression[[2]]$assay <- "PTEN activity"
dms_sig_regression[[3]]$assay <- "PTEN abundance"
...
dms_sig_regression$assay <- factor(dms_sig_regression$assay,
                                   levels = c("MSH2 activity", "PTEN abundance",
...
Plot a 'mutational signature' type barplot (bars = mean $\beta_{\text{signature}}$ over the datasets; transformed as $e^{-\beta_{\text{signature}}}$ so that those motifs more associated with damaging variants have taller bars).

```{r, fig.width=9.5, fig.height= 2.5}
ggplot(ddply(dms_sig_regression, c('variable', 'subs'), 
             summarise, coef = mean(coef)), 
       aes(x = variable, y = exp(-coef), fill = subs)) + 
  geom_bar(stat = 'identity') + facet_wrap(~ subs, nrow = 1, scales = "free_x") +
  cowplot::theme_cowplot() + 
  scale_y_continuous(name = bquote(e^ - beta[signature])) + xlab("") +
  scale_fill_manual(values = c("C>A" = "#1EBFF0", "C>G" = "#000000", 
                               "C>T" = "#FF0000", "T>A" = "#CBCACB", 
                               "T>C" = "#A1CF64", "T>G" = "#EDC8C5"))  +
